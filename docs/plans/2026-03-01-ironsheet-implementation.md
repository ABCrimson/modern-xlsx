# Ironsheet Phase 1: Core Read/Write Implementation Plan
...
resolver = "3"
members = [
    "crates/ironsheet-core",
    "crates/ironsheet-wasm",
]
...
repository = "https://github.com/nicholasgasior/ironsheet"
...
```json
{
  ...
  "scripts": {
    "build:wasm": "cd crates/ironsheet-wasm && wasm-pack build --target web --release --out-dir ../../packages/ironsheet/wasm",
    "build:ts": "pnpm -C packages/ironsheet build",
    "build": "pnpm build:wasm && pnpm build:ts",
    "test:rust": "cargo test --workspace",
    "test:ts": "pnpm -C packages/ironsheet test",
...
## Task 2: Create ironsheet-core Crate Skeleton

**Files:**
- Create: `crates/ironsheet-core/Cargo.toml`
- Create: `crates/ironsheet-core/src/lib.rs`
- Create: `crates/ironsheet-core/src/errors.rs`

**Step 1: Create Cargo.toml for ironsheet-core**
...
pub type Result<T> = std::result::Result<T, IronsheetError>;
```
...
pub use errors::{IronsheetError, Result};
```

**Step 4: Verify it compiles**

Run: `cargo check -p ironsheet-core`
Expected: Compiling ironsheet-core, Finished

**Step 5: Commit**

```bash
git add crates/ironsheet-core/
git commit -m "feat: add ironsheet-core crate with error types"
```

---

## Task 3: Create ironsheet-wasm Crate Skeleton

**Files:**
- Create: `crates/ironsheet-wasm/Cargo.toml`
- Create: `crates/ironsheet-wasm/src/lib.rs`

**Step 1: Create Cargo.toml for ironsheet-wasm**
...
[dependencies]
ironsheet-core = { path = "../ironsheet-core" }
...
Run: `cargo check -p ironsheet-wasm --target wasm32-unknown-unknown`
Expected: Compiling ironsheet-wasm, Finished

**Step 4: Build WASM with wasm-pack**

Run: `cd crates/ironsheet-wasm && wasm-pack build --target web --dev --out-dir ../../packages/ironsheet/wasm`
Expected: Successfully built WASM package

**Step 5: Commit**

```bash
git add crates/ironsheet-wasm/
git commit -m "feat: add ironsheet-wasm crate with wasm-bindgen skeleton"
```
...
**Files:**
- Create: `packages/ironsheet/package.json`
- Create: `packages/ironsheet/tsconfig.json`
- Create: `packages/ironsheet/tsdown.config.ts`
- Create: `packages/ironsheet/src/index.ts`
...
**Step 3: Create packages/ironsheet/package.json**
...
**Step 4: Create packages/ironsheet/tsconfig.json**
...
**Step 5: Create packages/ironsheet/tsdown.config.ts**
...
**Step 6: Create packages/ironsheet/src/index.ts**
...
Run: `pnpm -C packages/ironsheet build`
Expected: Build succeeds, dist/ created with index.mjs and index.d.ts

**Step 9: Commit**

```bash
git add tsconfig.base.json biome.json packages/ironsheet/ pnpm-lock.yaml
...
**Files:**
- Create: `crates/ironsheet-core/src/zip/mod.rs`
- Create: `crates/ironsheet-core/src/zip/reader.rs`
- Test: `crates/ironsheet-core/src/zip/reader.rs` (inline tests)

**Step 1: Write the failing test**

In `crates/ironsheet-core/src/zip/reader.rs`:
...
mod tests {
    ...
    fn test_rejects_path_traversal() {
        ...
        assert!(matches!(result.unwrap_err(), IronsheetError::Security(_)));
    }
...
Run: `cargo test -p ironsheet-core`
Expected: FAIL — module `zip` not found

**Step 3: Write the implementation**

`crates/ironsheet-core/src/zip/mod.rs`:
...
`crates/ironsheet-core/src/zip/reader.rs`:
...
use crate::errors::{IronsheetError, Result};
...
pub fn read_zip_entries(
    ...
    let mut archive = ZipArchive::new(reader)
        .map_err(|e| IronsheetError::ZipRead(e.to_string()))?;
    ...
    for i in 0..archive.len() {
        let mut entry = archive
            .by_index(i)
            .map_err(|e| IronsheetError::ZipEntry(e.to_string()))?;

        let name = entry.name().to_owned();

        // Security: reject path traversal
        if name.contains("..") || name.starts_with('/') || name.starts_with('\\') {
            return Err(IronsheetError::Security(format!(
...
```bash
git add crates/ironsheet-core/src/
...
**Files:**
- Create: `crates/ironsheet-core/src/zip/writer.rs`
- Modify: `crates/ironsheet-core/src/zip/mod.rs`

**Step 1: Write the failing test**

In `crates/ironsheet-core/src/zip/writer.rs`:
...
`crates/ironsheet-core/src/zip/writer.rs`:
...
pub fn write_zip(entries: &[ZipEntry]) -> Result<Vec<u8>> {
    ...
    for entry in entries {
        zip.start_file(&entry.name, options)
            .map_err(|e| IronsheetError::ZipWrite(e.to_string()))?;
    ...
    let cursor = zip
        .finish()
        .map_err(|e| IronsheetError::ZipFinalize(e.to_string()))?;
...
```bash
git add crates/ironsheet-core/src/zip/
...
**Files:**
- Create: `crates/ironsheet-core/src/ooxml/mod.rs`
- Create: `crates/ironsheet-core/src/ooxml/content_types.rs`

**Step 1: Write the failing test**

In `crates/ironsheet-core/src/ooxml/content_types.rs`:
...
`crates/ironsheet-core/src/ooxml/mod.rs`:
...
`crates/ironsheet-core/src/ooxml/content_types.rs`:
...
impl ContentTypes {
    ...
    pub fn to_xml(&self) -> Result<String> {
        ...
        writer
            ...
            .map_err(|e| IronsheetError::XmlWrite(e.to_string()))?;
        ...
        String::from_utf8(buf).map_err(|e| IronsheetError::XmlWrite(e.to_string()))
    }
}
...
**Files:**
- Create: `crates/ironsheet-core/src/ooxml/relationships.rs`
- Modify: `crates/ironsheet-core/src/ooxml/mod.rs`

**Step 1: Write the failing test**

In `crates/ironsheet-core/src/ooxml/relationships.rs`:
...
`crates/ironsheet-core/src/ooxml/relationships.rs`:
...
```bash
git add crates/ironsheet-core/src/ooxml/
...
**Files:**
- Create: `crates/ironsheet-core/src/ooxml/cell.rs`
- Modify: `crates/ironsheet-core/src/ooxml/mod.rs`

**Step 1: Write the failing test**

In `crates/ironsheet-core/src/ooxml/cell.rs`:
...
impl CellRef {
    ...
    pub fn parse(s: &str) -> Result<Self> {
        if s.is_empty() {
            return Err(IronsheetError::InvalidCellRef("empty cell reference".into()));
        }
        ...
        let row: u32 = row_str
            .parse()
            .map_err(|_| IronsheetError::InvalidCellRef(format!("invalid row number in '{s}'")))?;
...
**Files:**
- Create: `crates/ironsheet-core/src/ooxml/shared_strings.rs`
- Modify: `crates/ironsheet-core/src/ooxml/mod.rs`
...
`crates/ironsheet-core/src/ooxml/shared_strings.rs`:
        ...
        loop {
            match reader.read_event_into(&mut buf) {
                ...
                Ok(Event::Text(e)) if in_t => {
                    let text = e
                        .unescape()
                        .map_err(|err| IronsheetError::XmlParse(err.to_string()))?;
...
**Files:**
- Create: `crates/ironsheet-core/src/dates.rs`
- Modify: `crates/ironsheet-core/src/lib.rs`
...
`crates/ironsheet-core/src/dates.rs`:
...
fn serial_to_date_1900(serial: i64) -> Result<(i32, u32, u32)> {
    if serial < 0 {
        return Err(IronsheetError::InvalidDate(format!(
    ...
    let base = NaiveDate::from_ymd_opt(1900, 1, 1).ok_or_else(|| {
        IronsheetError::InvalidDate("cannot create base date 1900-01-01".into())
    })?;

    let target = base
        .checked_add_days(chrono::Days::new((adjusted - 1) as u64))
        .ok_or_else(|| {
            IronsheetError::InvalidDate(format!("serial number out of range: {serial}"))
        })?;
...
fn date_to_serial_1900(year: i32, month: u32, day: u32) -> Result<i64> {
    ...
    let base = NaiveDate::from_ymd_opt(1900, 1, 1)
        .ok_or_else(|| IronsheetError::InvalidDate("cannot create base date".into()))?;

    let target = NaiveDate::from_ymd_opt(year, month, day).ok_or_else(|| {
        IronsheetError::InvalidDate(format!("invalid date: {year}-{month}-{day}"))
    })?;
...
fn serial_to_date_1904(serial: i64) -> Result<(i32, u32, u32)> {
    ...
    let base = NaiveDate::from_ymd_opt(1904, 1, 1).ok_or_else(|| {
        IronsheetError::InvalidDate("cannot create base date 1904-01-01".into())
    })?;
...
```bash
git add crates/ironsheet-core/src/dates.rs crates/ironsheet-core/src/lib.rs
...
**Files:**
- Create: `crates/ironsheet-core/src/number_format.rs`
- Modify: `crates/ironsheet-core/src/lib.rs`
...
`crates/ironsheet-core/src/number_format.rs`:
...
```bash
git add crates/ironsheet-core/src/number_format.rs crates/ironsheet-core/src/lib.rs
...
**Files:**
- Create: `crates/ironsheet-core/src/ooxml/styles.rs`
- Modify: `crates/ironsheet-core/src/ooxml/mod.rs`
...
```bash
git add crates/ironsheet-core/src/ooxml/styles.rs
...
**Files:**
- Create: `crates/ironsheet-core/src/ooxml/workbook.rs`
- Modify: `crates/ironsheet-core/src/ooxml/mod.rs`
...
**Files:**
- Create: `crates/ironsheet-core/src/ooxml/worksheet.rs`
- Modify: `crates/ironsheet-core/src/ooxml/mod.rs`
...
**Files:**
- Modify: `crates/ironsheet-core/src/ooxml/worksheet.rs`
...
**Files:**
- Create: `crates/ironsheet-core/src/reader.rs`
- Modify: `crates/ironsheet-core/src/lib.rs`
...
**Files:**
- Create: `crates/ironsheet-core/src/writer.rs`
- Modify: `crates/ironsheet-core/src/lib.rs`
...
**Files:**
- Modify: `crates/ironsheet-wasm/src/lib.rs`
- Create: `crates/ironsheet-wasm/src/reader.rs`
- Create: `crates/ironsheet-wasm/src/writer.rs`
...
**Files:**
- Create: `packages/ironsheet/src/wasm-loader.ts`
- Modify: `packages/ironsheet/src/index.ts`

```typescript
let wasmModule: typeof import('../wasm/ironsheet_wasm.js') | null = null;

export async function initWasm(): Promise<void> {
  if (wasmModule) return;

  const wasm = await import('../wasm/ironsheet_wasm.js');
...
**Files:**
- Create: `packages/ironsheet/src/types.ts`
- Create: `packages/ironsheet/src/workbook.ts`
- Create: `packages/ironsheet/src/worksheet.ts`
- Create: `packages/ironsheet/src/cell.ts`
- Create: `packages/ironsheet/src/style.ts`
- Modify: `packages/ironsheet/src/index.ts`
...
**Files:**
- Create: `packages/ironsheet/__tests__/read.test.ts`
- Create: `packages/ironsheet/__tests__/write.test.ts`
- Create: `packages/ironsheet/__tests__/round-trip.test.ts`
- Create: `packages/ironsheet/__tests__/dates.test.ts`
...
Run:
```bash
cd crates/ironsheet-wasm && wasm-pack build --target web --release --out-dir ../../packages/ironsheet/wasm
```

**Step 2:** Check WASM size

Run: `ls -la packages/ironsheet/wasm/ironsheet_wasm_bg.wasm`
Expected: < 600KB uncompressed

Run: `gzip -c packages/ironsheet/wasm/ironsheet_wasm_bg.wasm | wc -c`
...
├── Task 2 (ironsheet-core skeleton) → Task 5 (ZIP reader)
│                                    → Task 6 (ZIP writer)
...
│
├── Task 3 (ironsheet-wasm skeleton) → Task 19 (WASM bridge) [depends on 17,18]
│
