...
pub fn read_zip_entries(data: &[u8], limits: &ZipSecurityLimits) -> Result<HashMap<String, Vec<u8>>> {
    ...
    let mut archive = ZipArchive::new(cursor).map_err(|e| IronsheetError::ZipRead(e.to_string()))?;
...
mod tests {
    ...
    fn test_rejects_path_traversal() {
        ...
        assert!(
            matches!(err, IronsheetError::Security(ref msg) if msg.contains("path traversal")),
    ...
    fn test_rejects_oversized_entries() {
        ...
        let err = result.unwrap_err();
        assert!(
            matches!(err, IronsheetError::Security(ref msg) if msg.contains("total decompressed size")),
